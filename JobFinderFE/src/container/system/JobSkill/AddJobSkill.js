...
const AddJobSkill = () => {
    ...
    const nameRegex = /^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠƯàáâãèéêìíòóôõọùúăđĩũơưỳỵỷỹýẠ-ỹ\s]+$/;

    let handleSaveJobSkill = async () => {
        setIsLoading(true)
        // Kiểm tra tính hợp lệ của tên kỹ năng
        if (!nameRegex.test(inputValues.name)) {
            setIsLoading(false);
            toast.error("Tên kỹ năng không hợp lệ. Vui lòng chỉ sử dụng chữ cái và khoảng trắng.");
            return;
        }
        if (isActionADD === true) {
            ...
            setTimeout(() => {
                ...
                else toast.error("Cập nhật kĩ năng thất bại")
            }, 50);
        }
    }
            ...
            <div className="col-12 grid-margin">
                <div className="card">
                    <div className="card-body">
                        ...
                        <h4 className="card-title">{isActionADD === true ? 'Thêm mới kĩ năng' : 'Cập nhật kĩ năng'}</h4>
                        <br></br>
